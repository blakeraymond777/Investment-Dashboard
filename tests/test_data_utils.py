...
def test_connect_uses_correct_filename():
    with patch("db_utils.sqlite3.connect") as mock_sqlite:
        mock_sqlite.return_value = MagicMock()
        result = db_utils.connect()
...
def test_fetch_nport_returns_none_when_no_url_in_db():
    with patch("finance_utils.db_utils.get_sec_url", return_value=None):
        result = data_scraping_utils.fetch_nport_from_sec_url("UNKNOWN")
...
def test_fetch_nport_fetches_and_returns_text():
    ...
    with patch("finance_utils.db_utils.get_sec_url", return_value="http://sec.gov/test"), \
         patch("finance_utils.data_scraping_utils.os.getenv", return_value="test@example.com"), \
         patch("finance_utils.data_scraping_utils.requests.get", return_value=mock_response) as mock_get:
        result = data_scraping_utils.fetch_nport_from_sec_url("VFIAX")
...
def test_load_funds_returns_dict(mock_conn):
    ...
    result = db_utils.load_funds_from_cache(["VFIAX", "FXAIX"], existing_connection=mock_conn)
...
def test_load_funds_returns_empty_dict_for_unknown_tickers(mock_conn):
    ...
    result = db_utils.load_funds_from_cache(["UNKNOWN"], existing_connection=mock_conn)
...
def test_load_funds_creates_connection_when_none_given():
    ...
    with patch("db_utils.connect", return_value=mock_conn) as mock_connect:
        db_utils.load_funds_from_cache(["VFIAX"])
...
def test_load_user_portfolio_returns_rows(mock_conn):
    ...
    result = db_utils.load_user_portfolio(1, existing_connection=mock_conn)
...
def test_load_user_portfolio_returns_empty_for_unknown_user(mock_conn):
    ...
    result = db_utils.load_user_portfolio(999, existing_connection=mock_conn)
...
def test_update_fund_raises_on_none_ticker():
    with pytest.raises(ValueError):
        db_utils.update_existing_fund(None)


def test_update_fund_no_op_when_no_fields_given(mock_conn):
    db_utils.update_existing_fund("VFIAX", existing_connection=mock_conn)
...
def test_update_fund_generates_correct_sql(mock_conn, field, value):
    ...
    db_utils.update_existing_fund("VFIAX", **kwargs)
...
def test_update_fund_closes_internally_created_connection():
    ...
    with patch("db_utils.connect", return_value=mock_conn):
        db_utils.update_existing_fund("VFIAX", sec_url="http://x.com")

    mock_conn.close.assert_called_once()


def test_update_fund_does_not_close_external_connection(mock_conn):
    db_utils.update_existing_fund("VFIAX", sec_url="http://x.com", existing_connection=mock_conn)
...
def test_get_sec_url_returns_url_when_found(mock_conn):
    ...
    result = db_utils.get_sec_url("VFIAX", existing_connection=mock_conn)
...
def test_get_sec_url_returns_none_when_not_found(mock_conn):
    ...
    result = db_utils.get_sec_url("UNKNOWN", existing_connection=mock_conn)
...
def test_delete_table_drops_named_table(mock_conn):
    db_utils.delete_table("portfolios", existing_connection=mock_conn)
...
def test_delete_table_skips_when_none(mock_conn):
    db_utils.delete_table(None, existing_connection=mock_conn)
    mock_conn.cursor().execute.assert_not_called()
...
def test_delete_user_portfolio_raises_on_none_user():
    with pytest.raises(ValueError):
        db_utils.delete_user_portfolio(None)


def test_delete_user_portfolio_executes_query(mock_conn):
    db_utils.delete_user_portfolio(1, existing_connection=mock_conn)
